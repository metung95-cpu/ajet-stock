...
with tab3:
    ...
    if not f_raw.empty:
        ...
        if sort_ord_t3 == "파란색 글자순 (미달 예상)":
            ...
            merged_df = merged_df.sort_values('_pacing_status', ascending=False) # 1, 0, -1
        else:
...
if not df_offer.empty and '보정오퍼가' in df_offer.columns:
    ...
    sorted_off_years = sorted(df_offer['연'].unique(), key=sort_numeric_string) if '연' in df_offer.columns else []
    sorted_off_months = sorted(df_offer['월'].unique(), key=sort_numeric_string) if '월' in df_offer.columns else []
