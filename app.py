...
COOKIE_NAME = "ajet_final_fix_v5" # 쿠키 이름 변경 (충돌 방지)

# ------------------------------------------------------------------
# 2. 쿠키 매니저 (모바일 최적화 및 오류 해결)
# ------------------------------------------------------------------
...
if not df.empty:
    ...
    if s_item: 
        f_df = f_df[f_df['품명'].str.contains(s_item, na=False)]
    if s_brand: 
        f_df = f_df[f_df['브랜드'].str.contains(s_brand, na=False, case=False)]
    ...
    if current_user == "AZS":
        ...
        if r_item: 
            t_df = t_df[t_df['품명'].str.contains(r_item, na=False)]
        if r_brand: 
            t_df = t_df[t_df['브랜드'].str.contains(r_brand, na=False, case=False)]
